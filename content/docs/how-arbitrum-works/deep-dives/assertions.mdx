...
Validators validate transactions by computing the next <Term id="chain-state">chain state</Term> using the chain's [STF](/how-arbitrum-works/deep-dives/01-stf-gentle-intro.mdx), whereas proposers can also assert and challenge the chain state on the parent chain.
