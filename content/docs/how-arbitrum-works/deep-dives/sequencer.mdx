...
- [Finality](#finality): A summary of the soft and <a data-quicklook-from="hard-finality">hard finality</a> levels, with a pointer to the full [Finality](/docs/how-arbitrum-works/deep-dives/finality) page (not a Sequencer task).
...
This section covers batch posting from the Sequencer's side. For the <a data-quicklook-from="batch-poster">batch poster</a>'s internals, see [The batch poster](/docs/how-arbitrum-works/deep-dives/batchposter). That page traces one batch from the transaction streamer to the parent chain transaction.
...
For the full treatment—latency at each stage, the trust model behind each level, force inclusion, AnyTrust data availability, and guidance on which level your application should wait for—see [Finality](/docs/how-arbitrum-works/deep-dives/finality).
