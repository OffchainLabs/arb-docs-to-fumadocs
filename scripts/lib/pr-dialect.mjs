 ...
 */
export function remapFrontmatter(source, { kind = 'doc', defaultAuthor = 'gblanchemain' } = {}) {
  const m = FRONTMATTER_RE.exec(source);
  if (!m) return source;
  const entries = parseFrontmatterEntries(m[1]);
  const body = source.slice(m[0].length);
  const rebuilt =
    kind === 'glossary' ? glossaryFrontmatter(entries) : docFrontmatter(entries, defaultAuthor);
  if (rebuilt === null) return source;
  return `---\n${rebuilt.join('\n')}\n---\n${body}`;
}

function docFrontmatter(entries, defaultAuthor) {
